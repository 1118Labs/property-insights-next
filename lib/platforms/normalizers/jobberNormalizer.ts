...
export function normalizeJobberClient(client: FSClientWithProperties): ClientRecord {
  return {
    id: client.id,
...
export function normalizeJobberProperty(property: FSProperty): PropertyRecord {
...
export function normalizeJobberJobs(jobs: FSJob[]): ServiceRequestRecord[] {
  return jobs.map((j) => ({
    id: j.id,
